...
class _StreamDemoOneState extends State<StreamDemoOne> {
  ...
  void initState() {
    super.initState();
    _streamController.stream.transform(StreamTransformer.fromHandlers(handleData: (vm, sink) {
      print(vm.age);
      vm.age++;
      sink.add(vm);
    })).listen((onData) {
        ...
        child: Column(children: [
          StreamBuilder<Good>(
              ...
              builder: (BuildContext context, AsyncSnapshot<Good> snapshot) {
                return Text('You hit me:${snapshot.data.name.toString()} ${snapshot.data.age.toString()} times');
              }),
          StreamBuilder<Good>(
              // 监听Stream，每次值改变的时候，更新Text中的内容
              stream: _streamController.stream,
